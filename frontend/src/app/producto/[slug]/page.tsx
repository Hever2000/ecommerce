...
import { Check, Ruler, Truck, RotateCcw, ShieldCheck } from 'lucide-react';

function slugToSegments(cat?: { slug: string; parent?: { slug: string; parent?: { slug: string } | null } | null } | null): string[] {
  if (!cat) return [];
  const slugs: string[] = [];
  let current: { slug: string; parent?: { slug: string; parent?: { slug: string } | null } | null } | null = cat;
  while (current) {
    slugs.unshift(current.slug);
    current = current.parent ?? null;
  }
  const segments: string[] = [slugs[0]];
  for (let i = 1; i < slugs.length; i++) {
    segments.push(slugs[i].replace(`${slugs[i - 1]}-`, ''));
  }
  return segments;
}
...
export default function ProductDetailPage() {
  ...
  if (error || !product) {
    return (
      <Container className="py-28 text-center">
        <h2 className="font-display text-4xl text-ink">{error ?? 'Producto no encontrado'}</h2>
        <p className="mt-4 text-ink-lighter">Este producto no existe o fue eliminado.</p>
  ...
  const categorySegments = product ? slugToSegments(product.category) : [];
  const categoryPath = categorySegments.length > 0 ? `/products/${categorySegments.join('/')}` : '/products';
    ...
    <div className="pt-28">
      <Container>
        <nav className="mb-8 flex items-center gap-2 text-xs text-ink-lighter">
          ...
          <span>/</span>
          <Link href="/products" className="hover:text-ink transition-colors">Productos</Link>
          {product.category && (
            <>
              <span>/</span>
              <Link href={categoryPath} className="hover:text-ink transition-colors">
        ...
        <div className="lg:grid lg:grid-cols-2 lg:gap-16">
          ...
          >
            ...
            <div className="mt-10 space-y-4 border-t border-cream-200 pt-8">
              {[
                { icon: Ruler, text: 'Encontrá tu talle perfecto — guía de talles.', link: 'Guía de Talles' },
                { icon: Truck, text: 'Envío gratis en órdenes de $150 o más.', link: 'Info de Envío' },
                { icon: RotateCcw, text: 'Devoluciones fáciles — 30 días.', link: 'Devoluciones' },
                { icon: ShieldCheck, text: 'Calidad premium garantizada.', link: 'Garantía' },
              ].map((item) => (
