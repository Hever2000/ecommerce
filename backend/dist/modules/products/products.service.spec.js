...
describe('ProductsService', () => {
    ...
    describe('findBySlug', () => {
        ...
        it('should throw NotFoundException when slug does not exist', async () => {
            prisma.product.findUnique.mockResolvedValue(null);
            await expect(service.findBySlug('nonexistent')).rejects.toThrow(common_1.NotFoundException);
        });
        it('should throw NotFoundException when product is soft-deleted', async () => {
            ...
            await expect(service.findBySlug('deleted-product')).rejects.toThrow(common_1.NotFoundException);
        });
    });
    describe('findOne', () => {
    ...
    describe('update', () => {
        ...
        it('should throw NotFoundException when updating non-existent product', async () => {
            prisma.product.findUnique.mockResolvedValue(null);
            await expect(service.update('nonexistent-id', updateDto)).rejects.toThrow(common_1.NotFoundException);
        });
    });
    describe('softDelete', () => {
        ...
        it('should throw NotFoundException when deleting non-existent product', async () => {
            prisma.product.findUnique.mockResolvedValue(null);
            await expect(service.softDelete('nonexistent-id')).rejects.toThrow(common_1.NotFoundException);
        });
    });
});
