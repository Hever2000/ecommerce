...
describe('InventoryService', () => {
    ...
    describe('adjust', () => {
        ...
        it('should decrease stock with type REMOVE', async () => {
            ...
            const result = await service.adjust({ variantId: mockVariant.id, type: 'REMOVE', quantity: 3 }, userId);
            expect(result.newStock).toBe(7);
        });
        it('should throw BadRequestException when REMOVE exceeds stock', async () => {
            prisma.productVariant.findUnique.mockResolvedValue(mockVariant);
            await expect(service.adjust({ variantId: mockVariant.id, type: 'REMOVE', quantity: 20 }, userId)).rejects.toThrow(common_1.BadRequestException);
        });
        it('should set exact stock with type SET', async () => {
            ...
            const result = await service.adjust({ variantId: mockVariant.id, type: 'SET', quantity: 25 }, userId);
            expect(result.newStock).toBe(25);
        });
        it('should throw BadRequestException when SET with negative stock', async () => {
            prisma.productVariant.findUnique.mockResolvedValue(mockVariant);
            await expect(service.adjust({ variantId: mockVariant.id, type: 'SET', quantity: -5 }, userId)).rejects.toThrow(common_1.BadRequestException);
        });
        it('should throw NotFoundException when variant does not exist', async () => {
            prisma.productVariant.findUnique.mockResolvedValue(null);
            await expect(service.adjust({ variantId: 'nonexistent-id', type: 'ADD', quantity: 5 }, userId)).rejects.toThrow(common_1.NotFoundException);
        });
        it('should create an inventory movement record', async () => {
            ...
            prisma.inventoryMovement.create.mockResolvedValue(mockMovement);
            await service.adjust({ variantId: mockVariant.id, type: 'ADD', quantity: 5, reason: 'Restock' }, userId);
        ...
        it('should throw BadRequestException for invalid adjustment type', async () => {
            prisma.productVariant.findUnique.mockResolvedValue(mockVariant);
            await expect(service.adjust({ variantId: mockVariant.id, type: 'INVALID', quantity: 5 }, userId)).rejects.toThrow(common_1.BadRequestException);
        });
    });
